...
class InlineEntityFormComplex extends InlineEntityFormBase implements ContainerFactoryPluginInterface {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    $wrapper = self::getWrapperId($this->getIefId());
    ...
    if (empty($open_form)) {
      ...
      elseif ($form_state->get(['inline_entity_form', $this->getIefId(), 'form']) == 'ief_add_existing') {
        $element['form'] = [
          ...
          // Identifies the IEF widget to which the form belongs.
          '#ief_id' => $this->getIefId(),
          // Used when multiple complex form at same page.
          '#ref_id' => $wrapper,
          // Used by Field API and controller methods to find the relevant
          // values in $form_state.
  ...
  public static function buildEntityFormActions(array $element) {
    ...
    $element['actions']['ief_' . $element['#op'] . '_save'] = [
      ...
      '#ajax' => [
        'callback' => 'inline_entity_form_get_element',
        'wrapper' => self::getWrapperId($element['#ief_id']),
      ],
    ];
  ...
  protected function buildRemoveForm(array &$form) {
    ...
    $form['actions']['ief_remove_confirm'] = [
      ...
      '#ajax' => [
        'callback' => 'inline_entity_form_get_element',
        'wrapper' => self::getWrapperId($form['#ief_id']),
      ],
  ...
  public static function submitConfirmRemove(array $form, FormStateInterface $form_state) {
    ...
    // If the entity has not been saved yet, then it won't have an ID.
    if ($entity !== FALSE && !$entity->isNew()) {
      $entity_id = $entity->id();
    }
  ...
  public static function getWrapperId($ief_id) {
    static $ids = [];
    if(!isset($ids[$ief_id])) {
      $ids[$ief_id] = Html::getUniqueId('inline-entity-form-' . $ief_id);
    }
    return $ids[$ief_id];
